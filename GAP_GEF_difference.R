...
gap_minus_gef_mut_order <- tag_avrg_gap_minus_gef %>% 
  arrange(gap_minus_gef_FC) %>% pull(mutant)

tag_avrg_gap_minus_gef %>% 
  mutate("mutant" = factor(mutant, gap_minus_gef_mut_order)) %>% 
  ggplot(aes(x = mutant, y = gap_minus_gef_FC, fill = tag)) + 
  geom_bar(stat = "identity") + 
  geom_errorbar(aes(ymin = gap_minus_gef_FC - gap_minus_gef_sd, ymax = gap_minus_gef_FC + gap_minus_gef_sd), width = 0.5) +
  theme(text = element_text(size = 20), axis.text.x = element_text(angle = 45, hjust = 1)) +
